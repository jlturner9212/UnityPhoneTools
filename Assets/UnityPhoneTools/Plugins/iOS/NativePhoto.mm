...
@interface UNativePhoto:NSObject
+ (int)checkPermission;
...
+ (char *)getImageProperties:(NSString *)path {
	...
	// To understand the magic numbers, see ImageOrientation enum in NativePhoto.cs
...
extern "C" int _NativePhoto_CheckPermission() {
	return [UNativePhoto checkPermission];
}

extern "C" int _NativePhoto_RequestPermission() {
	return [UNativePhoto requestPermission];
}

extern "C" int _NativePhoto_CanOpenSettings() {
	return [UNativePhoto canOpenSettings];
}

extern "C" void _NativePhoto_OpenSettings() {
	[UNativePhoto openSettings];
}

extern "C" void _NativePhoto_ImageWriteToAlbum(const char* path, const char* album) {
	[UNativePhoto saveMedia:[NSString stringWithUTF8String:path] albumName:[NSString stringWithUTF8String:album] isImg:YES];
}

extern "C" void _NativePhoto_VideoWriteToAlbum(const char* path, const char* album) {
	[UNativePhoto saveMedia:[NSString stringWithUTF8String:path] albumName:[NSString stringWithUTF8String:album] isImg:NO];
}

extern "C" void _NativePhoto_PickImage(const char* imageSavePath, int maxSize) {
	[UNativePhoto pickMediaSetMaxSize:maxSize];
	[UNativePhoto pickMedia:YES savePath:[NSString stringWithUTF8String:imageSavePath]];
}

extern "C" void _NativePhoto_PickVideo() {
	[UNativePhoto pickMedia:NO savePath:nil];
}

extern "C" int _NativePhoto_IsMediaPickerBusy() {
	return [UNativePhoto isMediaPickerBusy];
}

extern "C" char* _NativePhoto_GetImageProperties(const char* path) {
	return [UNativePhoto getImageProperties:[NSString stringWithUTF8String:path]];
}

extern "C" char* _NativePhoto_LoadImageAtPath(const char* path, const char* temporaryFilePath, int maxSize) {
	return [UNativePhoto loadImageAtPath:[NSString stringWithUTF8String:path] tempFilePath:[NSString stringWithUTF8String:temporaryFilePath] maximumSize:maxSize];
}